...
def precompute_distance_matrix():
	...
	# use the below snippet when computing the correlation
	
	np.save(os.path.join(root,'correlation.npy'), np.empty((num_rois, num_rois), dtype=np.float16))

	corr = np.memmap(os.path.join(root,'correlation.npy'), dtype='float16', mode='r+', shape=(num_rois,num_rois)) # write to here

	for index in range(num_rois):
	    corr[index] = np.corrcoef(test_statistic_1d[index],test_statistic_1d[:])[1,0].astype(np.float16)
