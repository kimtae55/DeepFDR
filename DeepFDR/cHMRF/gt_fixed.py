...
class GibbsSampler:
    ...
    def run(self, burn_in, num_samples):
        ...
        # initialize labels
        init_prob = np.full((self.SIZE, self.SIZE, self.SIZE), 0.5)
        ...
        while iteration < num_samples:
            ...
            label = sample(self.SIZE, self.params, label, self.dist)
    ...
    def precompute_distance_matrix(self):
        start = time.time()
        self.xyz = np.zeros((3, self.SIZE, self.SIZE, self.SIZE))
        for i in range(30):
            for j in range(30):
                for k in range(30):
                    self.xyz[0, i, j, k] = i
        ...
        dist = np.zeros((27000, 30, 30, 30))
        for i in range(30):
            for j in range(30):
                for k in range(30):
                    dist[30*30*i + 30*j + k] = d_ij(self.xyz[0], self.xyz[1], self.xyz[2], float64(i), float64(j), float64(k))
        end = time.time()
        print("time: ", end - start)        
        np.save(os.path.join(os.getcwd(), 'distance.npy'), dist)
