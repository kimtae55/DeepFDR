...
from methods import SingleTaskMethod, COSMOSMethod, UniformScalingMethod
...
def main(settings, args):
    ...
    logdir = os.path.join(args.datapath, settings['logdir'], settings['method'])
    pathlib.Path(logdir).mkdir(parents=True, exist_ok=True)


    # prepare
    train_set = utils.dataset_from_name(split='train', datapath=args.datapath, **settings)
    ###### val_set = utils.dataset_from_name(split='val', datapath=args.datapath, **settings)
    test_set = utils.dataset_from_name(split='test', datapath=args.datapath, **settings)
...
#---------------------------------------------------------------------------------------------------------------------
    ...
    test_loader = data.DataLoader(test_set, settings['batch_size'], shuffle=True, num_workers=settings['num_workers'])
    ...
    for j in range(settings['num_starts']):
        ...
        torch.save(method.model.state_dict(), os.path.join(logdir, str(settings['lr']) + '.pth'))
    return volume_max
...
def parse_args():
    ...
    parser.add_argument('--datapath', '-datapath', default=None, type=str, help='Specify datapath for deepFDR')
    parser.add_argument('--lr', '-lr', default=1e-3, type=float, help='Specify learning rate')
    ...
    elif args.dataset == 'deepFDR':
        settings.update(s.deepFDR)
