...
class Upsampler(nn.Module):
    ...
    def __init__(self, K, child_model, input_dim):
        ...
        if len(input_dim) == 1:
            ...
            self.three_d = True
            self.transposed_cnn = nn.Sequential(
                nn.ConvTranspose3d(K, K, kernel_size=4, stride=1, padding=0, bias=False),
                nn.ReLU(inplace=True),
                nn.ConvTranspose3d(K, K, kernel_size=6, stride=2, padding=1, bias=False),
                nn.ReLU(inplace=True),
                nn.Upsample(input_dim[-3:])
            )          
        else:
    ...
    def forward(self, batch):
        ...
        if self.three_d:
            # use transposed convolution
            a = a.reshape(b, len(batch['alpha']), 1, 1, 1)
            #print("a1", a.shape)
            a = self.transposed_cnn(a)
        #print(a.shape)
