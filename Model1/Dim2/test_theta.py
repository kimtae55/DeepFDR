...
class GibbsSampler:
    def __init__(self, rng_seed):
        ...
        self.SAVE_DIR = os.path.join(os.getcwd(), '../data/' + str(rng_seed) +'/label')
    ...
    def run(self, burn_in, n_iter):
        ...
        while iteration < n_iter:
            ...
            label = torch.unsqueeze(label, 0)
            sum_nn = torch.nn.functional.conv2d(label, kernel, stride=1, padding=1).squeeze()
            ...
            white = torch.bernoulli(conditional_distribution).cuda()
            white = torch.logical_and(white, self.white_map)
            #print(white)
            # save a pre_label 
            ...
            black_label = torch.logical_and(label.squeeze(), self.black_map).cuda()
            ...
            black = torch.logical_and(black, self.black_map)
            label = torch.logical_or(black, white).type(torch.FloatTensor).cuda()
            
            if iter_burn < burn_in:
                probOverTime[iter_burn] = torch.count_nonzero(label) / (Data.VOXEL_SIZE * Data.VOXEL_SIZE)
...
if __name__ == "__main__":
    ...
    print("VOXEL_SIZE: ", Data.VOXEL_SIZE, Data.VOXEL_SIZE)
