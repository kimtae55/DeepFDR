...
class Model1:
    def __init__(self, x_file, rng_seed):
        ...
        self.params = {'B': 0,
                       ...
                       'B_prev': 0,
                       'H_prev': 0,
                       'pL': torch.tensor([0.3,0.7]),
                       'muL': torch.tensor([1,1.5]),
                       'sigmaL2': torch.tensor([1.5,1.5]),
        ...
        self.const = {'a': 1,  # for penalized likelihood for L >= 2
                      ...
                      'alpha': 1e-4,
                      'burn_in': 1000, # 1000
                      'num_samples': 100, # 5000
                      'L': 2,
                      'newton_max': 3,
                      'fdr_control': 0.1,
                      'tiny': 1e-8
                      }
        self.x = torch.from_numpy(np.loadtxt(x_file).reshape((Data.VOXEL_SIZE,Data.VOXEL_SIZE,Data.VOXEL_SIZE))).float().cuda()
        ...
        self.white_map = torch.from_numpy(np.indices((30,30,30)).sum(axis=0) % 2)
        self.white_map = self.white_map.cuda()
        ones = torch.ones((30,30,30,)).cuda()
    ...
    def gem(self):
        ...
        for t in range(self.const['maxIter']):
            ...
            for l in range(self.const['L']):
                ...
                self.params['sigmaL2'][l] = (2 * self.const['a'] + self.sum_sigmaL2(omega[l], self.x,
                                                                                    self.params['muL'][l])) \
