...
class UEnc(nn.Module):
    ...
    def forward(self, x):
        ...
        middle = self.middle(F.max_pool3d(enc2, (2,2,2)))
        ...
        dec4=self.dec4(up4, lastLayer=True)
...
class WNet(nn.Module):
    def __init__(self, squeeze, ch_mul=64, in_chans=1, out_chans=1):
        super(WNet, self).__init__()
        self.UEnc=UEnc(squeeze, ch_mul, in_chans)
        self.UDec=UDec(squeeze, ch_mul, out_chans)
        self.p3d = (1, 1, 1, 1, 1, 1)

    def forward(self, x, returns='both', qv=None):
        enc = torch.sigmoid(self.UEnc(x)) # P(h|x), gamma
        ...
        enc = F.pad(unpadded_enc, self.p3d, "constant", 0)
