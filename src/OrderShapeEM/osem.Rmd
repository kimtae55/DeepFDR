...
mu_paths <- c('/mu/mu_n4_2', '/mu/mu_n35_2', '/mu/mu_n3_2', '/mu/mu_n25_2', '/mu/mu_n2_2', '/mu/mu_n15_2', '/mu/mu_n1_2', '/mu/mu_n05_2', '/mu/mu_n0_2')
...
run_osem <- function(path) {
  ...
  for (iteration in 1:num_iterations) {
    ...
    x.vec <- read.table(paste(root, path, "/data0.2.txt", sep = ""))[start_index:end_index, ]
    x <- array(x.vec, dims)
    pv <- 2 * pnorm(-abs(x), 0, 1)
    qv <- qvalue(pv, threshold)
    adjusted_p_values <- p.adjust(pv, method = "BH")
  
    # run osem here
    osem <-  OrderShapeEM(pv, adjusted_p_values, OrderShapeEM.control(trace = TRUE))
    osem_signal <- osem$fdr <= threshold
    
    label <- read.table(paste(root, "/cubes0.2.txt", sep = ""))
    label <- label$V1
    num_rejected <- length(which(osem_signal != 0))
    num_not_rejected <- (30 * 30 * 30) - num_rejected

    # calculate fdr, fnr, atp
    fdr <- sum(osem_signal == 1 & label == 0)
    atp <- sum(osem_signal == 1 & label == 1)
    fnr <- sum(osem_signal == 0 & label == 1)
  ...
  write.table(df, file = paste0(root, path, "/osem_pv0.2.txt"), append = TRUE, sep = " ", dec = ".",
