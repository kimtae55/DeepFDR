...
class Model1:
    def __init__(self, x_file, rng_seed, savepath):
        ...
        init_prob = torch.full(self.dim, 0.5)
        self.init = torch.bernoulli(init_prob).cuda()  # p = 0.5 to choose 1
        
        self.start = time.time()
        self.end = 0
        
        self.white_map = torch.from_numpy(np.indices(self.dim).sum(axis=0) % 2).type(torch.FloatTensor)
        self.white_map = self.white_map.cuda()
        ones = torch.ones(self.dim).cuda()
    ...
    def gem(self):
        ...
        for t in range(self.const['maxIter']):
            ...
            omega = torch.empty((2,)+self.dim).cuda()
...
def p_lis(datapath, savepath, threshold=0.001):
    '''
    Rejection of null hypothesis are shown as 1, consistent with online BH, Q-value, smoothFDR methods.
    # LIS = P(theta = 0 | x)
    # gamma_1 = P(theta = 1 | x) = 1 - LIS

    Since this is pooled LIS, take all gammas within the folder, then extract ROI-based gammas, then sort them to calculate the LIS
    '''
    roi = nib.load('/scratch/tk2737/DeepFDR/data/sim/ADNIdata/atlas-AAL2_dseg.nii.gz')
    roi_map = np.array(roi.dataobj)
    roi_labels = np.unique(roi_map)
    roi_labels = roi_labels[roi_labels != 0]  # Exclude the background label (0)

    dtype = [('index', int), ('value', float)]

    gamma_3d = np.zeros(roi_map.shape)
    with open(datapath, 'rb') as file:
        gamma_cubes = pickle.load(file)

        for label in roi_labels:
            positions = np.argwhere(roi_map == label)
            for pos in positions:
                adjusted_pos = pos - gamma_cubes[label][1]
                gamma_3d[pos[0], pos[1], pos[2]] = gamma_cubes[label][0][adjusted_pos[0], adjusted_pos[1], adjusted_pos[2]]

    # Now perform FDR control using gamma_3d --> gamma_1d
    gamma_1d = gamma_3d.ravel()
    size = gamma_1d.size
    lis = np.zeros(size, dtype=dtype)
    for i in range(size):
        lis[i]['index'] = i
        lis[i]['value'] = 1-gamma_1d[i]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--plis_savepath', default=None, type=str)
    parser.add_argument('--gamma_savepath', default=None, type=str)
    ...
    gamma_pickle = open(args.gamma_savepath, "wb")
    gamma_cubes = {}

    with open(args.datapath, 'rb') as file:
        roi_cubes = pickle.load(file)

        for label in roi_labels:
            x_file = np.array(roi_cubes[label][0])
            fdr = Model1(x_file, args.seed, args.savepath)
            gamma = fdr.gem()

            gamma_cubes[label] = (gamma, roi_cubes[label][1], roi_cubes[label][2])
            break

    pickle.dump(gamma_cubes, gamma_pickle)
    gamma_pickle.close()

    p_lis(args.gamma_savepath, args.plis_savepath)
